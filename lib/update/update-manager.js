...
const runUpdateEmployeeManager = async () => {
  try {
    ...
    let sql1 = `SELECT CONCAT(employee.first_name, ' ', employee.last_name) AS employee FROM employee`;
    let allEmployees = await db.promise().query(sql1);
    allEmployees = allEmployees[0].map((res) => res.employee);

    // Get the names of all managers in the database
    let sql2 = `SELECT CONCAT(employee.first_name, ' ', employee.last_name) AS manager FROM employee`;
    let allManagers = await db.promise().query(sql2);
    ...
    updateEmployeeManager(employee_to_update, managerId, manager_assigned);
  } catch (error) {
    console.log(error);
  }
};
...
const updateEmployeeManager = async (employee, manager, manager_assigned) => {
